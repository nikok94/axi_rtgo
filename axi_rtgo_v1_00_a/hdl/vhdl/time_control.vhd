------------------------------------------------------------------------------
-- time_control.vhd - entity/architecture pair
------------------------------------------------------------------------------
--
-- ***************************************************************************
...
--
------------------------------------------------------------------------------
-- Filename:          time_control.vhd
-- Version:           1.00.a
-- Description:       User logic.
-- Date:              Fri Jul 20 08:51:19 2018 (by Create and Import Peripheral Wizard)
...
library axi_rtgo_v1_00_a;
use axi_rtgo_v1_00_a.lsync;
...
end entity time_control;

------------------------------------------------------------------------------
-- Architecture section
------------------------------------------------------------------------------

architecture IMP of time_control is

component gray_cnt_v3 is
    generic (
      SIZE            : integer := 64
    );
    port (
      clk             : in std_logic;
      nreset          : in std_logic;
      init_count_bin  : in std_logic_vector(63 downto 0);
      q               : out std_logic
    );
end component gray_cnt_v3;

  ------------------------------------------
  -- Signals for user logic slave model s/w accessible register example
  ------------------------------------------
  signal init_time_31_0                 : std_logic_vector(C_SLV_DWIDTH-1 downto 0):= (others=>'0');
  ...
  signal time_out_cntrl                 : std_logic_vector(63 downto 0);

begin
    TIME_AXIS_TDATA <= time_out_cntrl;
    TIME_AXIS_TLAST <= '1';
    TIME_AXIS_TVALID <= not soft_rst_s;

   process (time_axis_aclk)
   begin
      if rising_edge(time_axis_aclk) then
         if Bus2IP_Resetn = '0' then
            soft_rst <= '1';
         else
            soft_rst <= not contr_reg(0);
         end if;
      end if;
   end process;

lsync_inst: entity axi_rtgo_v1_00_a.lsync
   generic map(
      sync_rst => false                              -- true for synchronous reset type
   ...
   port map(
    ...
    begin
        if rising_edge(time_axis_aclk) then
            if soft_rst_s = '1' then
                time_out_cntrl <= init_time_63_32 & init_time_31_0;
                time_valid <= '0';
            else
                time_valid <= '1';
                time_out_cntrl <= time_out_cntrl + 1;
            end if;
        end if;
    end process;
  ------------------------------------------
      ...
      if Bus2IP_Resetn = '0' then
        ...
        case slv_reg_write_sel is
          ...
          when "010000" =>
            for byte_index in 0 to (C_SLV_DWIDTH/8)-1 loop
              if ( Bus2IP_BE(byte_index) = '1' ) then
                init_time_31_0(byte_index*8+7 downto byte_index*8) <= Bus2IP_Data(byte_index*8+7 downto byte_index*8);
              end if;
            end loop;
          when "001000" =>
            for byte_index in 0 to (C_SLV_DWIDTH/8)-1 loop
  ...
  SLAVE_REG_READ_PROC : process( slv_reg_read_sel, init_time_31_0, init_time_63_32, contr_reg, slv_reg3, slv_reg4, slv_reg5 ) is
  begin

    case slv_reg_read_sel is
      when "100000" => slv_ip2bus_data <= init_time_31_0;
